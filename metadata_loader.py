...
class MetadataBasedDataLoader:
    ...
    def run_pipeline(self, csv_file: str = None, output_formats: List[str] = None) -> bool:
        ...
        Args:
            csv_file (str): Path to CSV file
            output_formats (List[str]): Output formats ['csv', 'json', 'sqlite', 'oracle', 'excel']
...
if __name__ == "__main__":
    ...
    # Option 1: Run pipeline with local outputs (SQLite, CSV, JSON, Excel)
    success = loader.run_pipeline(
        csv_file='emp.csv',
        output_formats=['csv', 'json', 'sqlite', 'excel']
    )
    
    # Option 2: To use Oracle database, configure Oracle connection:
    # oracle_config = {
